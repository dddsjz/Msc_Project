...
def run(mode = 'm', subject = 1, test_sub=-1, points = 200, minutes = 200/60, ratio = 0.7):
    ...
    # generate origin ECG and test set
        ...
        # generate train data
        dp = DataProcess('bidmc_1_Signals.csv', minutes, cols_to_use=[' PLETH', ' II'])
        data = dp.data
        ...
        for i in range(1, 53):
            dp = DataProcess('bidmc_' + str(i+1) + '_Signals.csv', minutes, cols_to_use=[' PLETH', ' II'])
            data = dp.data
            signals = dp.process_data_origin(data, points)
    ...
    for i in range(0, len(test_PPGset)):
        ...
        np.savetxt(current+'/data/txt/city/test_b4c'+str(i+1)+'.txt', out, fmt = '%d', delimiter = ', ')
