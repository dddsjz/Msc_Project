...
class DataProcess(object):
    ...
    def __init__(self, filename, minutes, frequency = 100):
        current = os.getcwd()
...
def run(mode = 'm', subject = 1, test_sub=-1, points = 200, minutes = 200/60, ratio = 0.7):
    ...
    if subject <= 0 or subject > 25 or (mode != 's' and mode != 'm') or test_sub > 25:
        print('Subject number and test_sub number should in range 1-25, and mode can only be singal(s) or multi(m)')
        return 0

    if mode == 'm':  # mulit subject
        dp = DataProcess('0.csv', minutes)
        data = dp.data
        ...
        if ecg_length < 40:
            ...
            out = array.T
            current = os.getcwd()
            # np.savetxt(current+'/data/txt/city/train_'+str(1)+'.txt', out, fmt = '%d', delimiter = ', ')
        ...
        num = 1
        for i in range(1, 25):
            dp = DataProcess(str(i)+".csv", minutes)
            data = dp.data
            signals = dp.process_data_origin(data, points)
            ...
            out = array.T
            current = os.getcwd()            
            # np.savetxt(current+'/data/txt/city/train_'+str(num+1)+'.txt', out, fmt = '%d', delimiter = ', ')
        ...
        if test_sub != -1:
            dp1 = DataProcess(str(test_sub-1) + '.csv', minutes)
            data1 = dp1.data
            test_PPGset = []
        ...
        dp = DataProcess(str(subject-1) + '.csv', minutes)
        data = dp.data
        test_ECGset = []
        test_PPGset = []
        thr = 0.02
        ...
        if signals == 0:
            print('the sample '+ str(subject-1) +' is too noisy.')
            return 0
        ...
        out = array.T
        current = os.getcwd()            
        np.savetxt(current+'/data/txt/city/train_'+str(subject-1)+'.txt', out, fmt = '%d', delimiter = ', ')
        ...
        if signals == 0:
            print('the sample '+ str(test_sub-1) +' is too noisy.')
            return 0
    ...
    for i in range(0, len(test_PPGset)):
        ...
        tmp1 = np.asarray(test_ECGset[i]).flatten()
        
        if i == 0:
            gen_ECG = pd.DataFrame(tmp)
            ori_ECG = pd.DataFrame(tmp1)

        else:
            gen_ECG = pd.concat([gen_ECG, pd.DataFrame(tmp)], ignore_index=True, sort=False)
            ori_ECG = pd.concat([ori_ECG, pd.DataFrame(tmp1)], ignore_index=True, sort=False)
        ...
        current = os.getcwd()
        if test_sub != -1:
            np.savetxt(current+'/data/txt/city/test_cm'+str(test_sub)+'.txt', out, fmt = '%d', delimiter = ', ')
            pass
        elif mode == 's':
            np.savetxt(current+'/data/txt/city/test_cs'+str(subject)+'.txt', out, fmt = '%d', delimiter = ', ')
            pass
        else:
            # np.savetxt(current+'/data/txt/city/test_'+str(i+1)+'.txt', out, fmt = '%d', delimiter = ', ')
            pass

    # plot result
    tmp = np.asarray(gen_ECG)
    tmp = tmp.flatten()
    tmp = tmp.tolist()
    plt.plot(tmp, 'r')

    tmp1 = np.asarray(ori_ECG)
    tmp1 = tmp1.flatten()
    tmp1 = tmp1.tolist()
    # plt.plot(tmp1, 'b')
    ...
    # ft_ecg = abs(np.fft.fft(tmp1))  # original ECG
    # ft_test = ft_ecg[range(1, int(len(ft_ecg)/2))]
    # plt.plot(ft_test, 'b')

    # ft_ecg = abs(np.fft.fft(tmp))  # generated ECG
    # ft_test = ft_ecg[range(1, int(len(ft_ecg)/2))]
    # xlabel = np.arange(1, 5000)
    # plt.plot(ft_test, 'r-.')
...
if __name__ == '__main__':
    ...
    # run(mode = 'm', test_sub = 2, points = 100, minutes = 300/60, ratio = 0.5)

    # for i in range (1, 26):
        # run(mode = 's', subject = i, points = 100, minutes = 300/60, ratio = 0.5)

    run(mode = 's', subject = 12, test_sub = 2, points = 100, minutes = 300/60, ratio = 0.5)  # only get performance.