...
def run(mode = 'm', subject = 1, test_sub=-1, points = 200, minutes = 200/60, ratio = 0.7):
    ...
    for i in range(0, len(test_PPGset)):
        ...
        current = os.getcwd()
        if test_sub != -1:
            np.savetxt(current+'/data/txt/biodb/test_bm'+str(test_sub)+'.txt', out, fmt = '%d', delimiter = ', ')
            pass
        elif mode == 's':
            np.savetxt(current+'/data/txt/biodb/test_bs'+str(subject)+'.txt', out, fmt = '%d', delimiter = ', ')
            pass
        else:
            # np.savetxt(current+'/data/txt/biodb/test_'+str(i+1)+'.txt', out, fmt = '%d', delimiter = ', ')
            pass
